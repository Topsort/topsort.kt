...
class CacheTest {
    ...
    fun setup() {
        ...
        Cache.setup(
            context = context,
            identity = requireNotNull(UserIdentity.Marketplace.of("test-user-id")),
            token = "test-token"
        )
    }
    ...
    fun setup_stores_token() {
        Cache.setup(context, requireNotNull(UserIdentity.Marketplace.of("user-123")), "my-api-token")
    ...
    fun setup_can_update_token() {
        Cache.setup(context, requireNotNull(UserIdentity.Marketplace.of("user-1")), "token-1")
        Cache.setup(context, requireNotNull(UserIdentity.Marketplace.of("user-2")), "token-2")
    ...
    fun stored_events_readable_after_setup_called_again() {
        ...
        Cache.setup(context, requireNotNull(UserIdentity.Marketplace.of("test-user-id")), "test-token")
